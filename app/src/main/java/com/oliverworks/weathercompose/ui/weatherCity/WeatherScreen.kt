...
import com.oliverworks.weathercompose.R
import com.oliverworks.weathercompose.util.Status
...
fun WeatherScreen(city: String, viewModel: WeatherViewModel = hiltViewModel()) {
    ...
    Column(
        ...
        when (viewModel.cityWeather.value.status) {
            Status.SUCCESS -> {
                Text(text = viewModel.cityWeather.value.data?.name ?: city, fontSize = 28.sp)
                Image(
                    ...
                    ), contentDescription = "", modifier = Modifier.size(256.dp)
                )
                Text(
                ...
                Text(
                    buildAnnotatedString {
                        withStyle(style = SpanStyle(color = Color.Blue)) {
                            append("${viewModel.cityWeather.value.data?.main?.tempMin}°")
                        }
                        append(" | ")
                        withStyle(
                            style = SpanStyle(
                            ...
                            append("${viewModel.cityWeather.value.data?.main?.tempMax}°")
                        }
                    }, fontSize = 32.sp
                )
                Text(
